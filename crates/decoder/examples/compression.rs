...
//!
//! Prerequisites:
//! A v0 dbin file both compressed and uncompressed, containing Ethereum content.
...
fn main() {
    ...
    let mut block = blocks_compressed.get(0).unwrap();
    assert!(matches!(block, AnyBlock::Eth(_)));
    ...
    block = blocks_decompressed.get(0).unwrap();
    assert!(matches!(block, AnyBlock::Eth(_)));
