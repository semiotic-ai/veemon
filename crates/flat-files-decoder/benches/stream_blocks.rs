...
use flat_files_decoder::{dbin::read_message_from_stream, error::DecoderError};
use prost::Message;
...
fn read_decode_check_bench(c: &mut Criterion) {
    ...
    group.bench_function("read-message-stream", |b| {
        ...
        for file in files {
            ...
            loop {
                b.iter(|| {
                    message = black_box(read_message_from_stream(&mut reader));
                });
                match message {
                    Ok(_) => continue,
                    Err(_) => {
    ...
    group.bench_function("decode-bstream", |b| {
        ...
        for file in files {
            ...
            let mut reader = BufReader::new(file);
            loop {
                let message = match read_message_from_stream(&mut reader) {
                    Ok(message) => message,
                    Err(_) => {
                        break;
                    }
